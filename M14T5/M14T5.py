...
[builder_.button(text=f'Product{i}', callback_data='product_buying') for i in range(1, 5)]
ikb_buy_menu = builder_.as_markup()
...
async def set_weight(message: Message, state: FSMContext) -> None:
    ...
    if message.text.isdigit():
        ...
        result: float = float(data["weight"]) * 10 + float(data["growth"]) * 6.25 - float(data["age"]) * 5 + 5
...
async def set_username(message: Message, state: FSMContext) -> None:
    if isLatin(message.text):
        ...
        await message.answer('Используйте латинский алфавит, еще раз:')
...
async def command_start_handler(message: Message) -> None:
    ...
    await message.answer("Привет! Я бот помогающий твоему здоровью.",
                         reply_markup=kb_menu(['Рассчитать', 'Информация', 'Купить', 'Регистрация']))
...
@dp.message(F.text.lower().startswith('регистрация'))
...
def isLatin(text: str) -> bool:
    """
    Проверяет соответствие всех символов строки латинскому алфавиту
    :param text: Искомая строка
    :return: Соответстствие
    """
    for ch in text:
...
def isEmail(email: str) -> bool:
    """
    Проверяет синтаксис написания e-mail
    :param email: e-mail
    :return: Соответствие
    """
    if email[0].isalpha() and email[-1].isalpha():
        i: int = email[1:].find('@')
        if i != -1 and email[i + 2].isalpha():
            i: int = email[i + 3:].find('.')
            if i != -1:
                return True
    return False
